...
/*   Updated: 2024/02/14 11:12:48 by seojilee         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	build_cmd_pack(t_ast *tree, t_env_pack *pack, t_cmd *cmd)
{
	...
	while (cur && cur <= tree->end)
	{
		if (cur->prev && cur->prev->type == T_D_LESSER)
		{
			...
			append_redir(cmd, cur->prev->type, expand(cur->string, pack, 1));
		else if (cur->type == T_WORD)
			cmd->c_args = merge_strs(cmd->c_args, expand(cur->string, pack, 0));
	}
}
